...
class InmunizacionesController extends Controller
{
    public function index($id)
    {
        ...
        return view('usuario.monitoreo.modulos.inmunizaciones', compact('acta', 'detalle', 'equipos'));
    }
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            if (($datos['utiliza_sihce'] ?? '') === 'NO') {
                ...
                $datos['utiliza_reportes']        = null;
                $datos['socializa_reportes']      = null;
            ...
            // REGLA D: Si NO utiliza reportes -> Limpiar institución
            if (isset($datos['utiliza_reportes']) && $datos['utiliza_reportes'] === 'NO') {
                $datos['socializa_reportes'] = null;
            }
            ...
            // Si dice que NO es compartido, forzamos a null el campo "con quien"
            if (isset($datos['es_compartido']) && $datos['es_compartido'] === 'NO') {
            ...
            return redirect()->route('usuario.monitoreo.modulos', $id)
                             ->with('success', 'Módulo 09 (Inmunizaciones) sincronizado correctamente.');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Error Módulo Inmunizaciones (Store) - Acta {$id}: " . $e->getMessage());
