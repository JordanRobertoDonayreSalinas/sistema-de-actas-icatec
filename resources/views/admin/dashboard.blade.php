...
@section('header-content')
    <h1 class="text-xl font-bold text-slate-800 tracking-tight">Bienvenido de nuevo, {{ Auth::user()->name }}</h1>
...
@section('content')
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        {{-- 1. SECCIÓN MÉTRICAS --}}
        <div>
            <h3 class="text-sm font-bold text-slate-500 uppercase tracking-widest mb-4">Estadísticas de Operación</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {{-- POSICIÓN 2: RANKING ESTABLECIMIENTOS (VIOLETA) --}}
                <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 p-8 text-white shadow-xl shadow-indigo-500/10 group transition-transform hover:scale-[1.01]">
                    ...
                    <div class="relative z-10 flex flex-col justify-between h-full">
                        ...
                        <div>
                            ...
                            <div class="h-40 overflow-y-auto custom-scroll pr-2 space-y-2">
                                @forelse($topEstablecimientos as $establecimiento)
                                    <div class="flex justify-between items-center bg-black/10 rounded px-3 py-2 text-sm hover:bg-black/20 transition-colors">
                                        <span class="font-medium truncate w-2/3" title="{{ $establecimiento->nombre }}">{{ $establecimiento->nombre }}</span>
                                        <div class="flex items-center gap-2">
                                            <span class="text-[10px] text-indigo-200">Actas:</span>
                                            <span class="bg-white text-purple-700 font-bold px-2 py-0.5 rounded text-xs min-w-[30px] text-center">
                                                {{ $establecimiento->actas_count }}
                                            </span>
                                        </div>
                                    </div>
                                @empty
                                    <p class="text-sm text-indigo-100 italic">No hay actividad registrada aún.</p>
                                @endforelse
                            </div>
                        </div>
                    </div>
