...
{{-- CAMBIOS DE ESTILO: Se reemplazó 'rose' por 'slate' y 'teal' para coincidir con los demás componentes --}}
<div id="card_admin_{{$prefix}}" class="bg-white border border-slate-200 rounded-[3rem] overflow-hidden shadow-xl shadow-slate-200/40 transition-all duration-700 mb-10 group/card relative">
    ...
    {{-- BARRA LATERAL (Gris suave para diferenciar del verde del componente principal) --}}
    ...
    <div class="bg-slate-50/50 border-b border-slate-100 px-10 py-6 flex flex-col lg:flex-row justify-between items-center gap-6">
        <div class="flex items-center gap-5">
            {{-- ICONO: Fondo blanco y borde slate, icono Teal --}}
            ...
            <div>
                {{-- TÍTULO: Teal Oscuro para consistencia --}}
                ...
                <p class="text-slate-400 font-bold uppercase text-[10px] tracking-widest">
                    Accesos SIHCE y Declaraciones Juradas
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="p-10 pl-16">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
                ...
                <div class="relative">
                    {{-- INPUTS: Fondo Slate-50, Borde Slate-100, Focus Teal --}}
            ...
            <div id="div_firmo_dj_{{$prefix}}" 
                 class="transition-all duration-500 ease-out {{ $sihce == 'NO' ? 'opacity-40 grayscale pointer-events-none' : '' }}">
                <label class="block text-slate-400 text-[10px] font-black uppercase tracking-widest mb-3 ml-1">
...
<script>
    /**
     * Muestra/Oculta/Deshabilita los documentos administrativos según si tiene SIHCE o no
     */
    function toggleDocAdmin(valor, prefix) {
        ...
        if (valor === 'NO') {
            divDj.classList.add('opacity-40', 'grayscale', 'pointer-events-none');
            divConf.classList.add('opacity-40', 'grayscale', 'pointer-events-none');
            
            selDj.value = 'NO'; 
            selConf.value = 'NO'; 
        } else {
            divDj.classList.remove('opacity-40', 'grayscale', 'pointer-events-none');
            divConf.classList.remove('opacity-40', 'grayscale', 'pointer-events-none');
            
            if(selDj.value === 'NO' && valor === '') selDj.value = '';
            if(selConf.value === 'NO' && valor === '') selConf.value = '';
        }
    }
