...
@section('header-content')
    ...
    <div class="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
        ...
        <span class="font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100">Borrador Independiente</span>
        <span class="text-slate-300">•</span>
...
<div id="page-wrapper" class="max-w-7xl mx-auto space-y-5">
    ...
    <div class="bg-indigo-600 rounded-2xl p-4 shadow-lg flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-indigo-100 text-[11px]">Cualquier cambio de sector aquí <b>no afecta</b> al cronograma oficial principal.</p>
            </div>
        </div>
        <div class="hidden md:block">
             <span class="text-[10px] bg-white/10 text-white px-3 py-1.5 rounded-lg border border-white/20 font-bold uppercase tracking-widest">Entorno Seguro</span>
        </div>
    </div>
    ...
    <div id="seccion-filtros" class="bg-white rounded-2xl shadow-sm border border-slate-200 transition-all duration-300">
        <div class="flex items-center justify-between px-5 py-3 border-b border-slate-100 cursor-pointer hover:bg-slate-50 transition-colors" onclick="document.getElementById('filtros-content').classList.toggle('hidden'); document.getElementById('icon-filtros-toggle').classList.toggle('rotate-180');">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-[11px] font-black text-slate-600 uppercase tracking-widest">Filtrar para Re-Sectorizar</h3>
            </div>
        ...
        <div id="filtros-content" class="p-5 hidden">
            <div class="flex flex-wrap items-end gap-3">
                <div class="flex flex-col gap-1">
                    ...
                    <select id="filtro-provincia" class="text-xs border-slate-200 rounded-xl px-3 py-2.5 font-bold text-slate-700 bg-slate-50 min-w-[130px]">
                        <option value="">Todas</option>
                    </select>
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-[9px] font-black text-slate-400 uppercase">Distrito</label>
                    <select id="filtro-distrito" class="text-xs border-slate-200 rounded-xl px-3 py-2.5 font-bold text-slate-700 bg-slate-50 min-w-[150px]">
                        <option value="">Todos</option>
                    </select>
                </div>
                <div class="flex flex-col gap-1 flex-1 min-w-[200px]">
                    <label class="text-[9px] font-black text-slate-400 uppercase">Establecimiento</label>
                    <select id="filtro-establecimiento" class="text-xs border-slate-200 rounded-xl px-3 py-2.5 font-bold text-slate-700 bg-slate-50 w-full">
                        <option value="">Todos</option>
                    </select>
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-[9px] font-black text-slate-400 uppercase">Sector</label>
                    <select id="filtro-sector" class="text-xs border-slate-200 rounded-xl px-3 py-2.5 font-bold text-slate-700 bg-slate-50 min-w-[130px]">
                        <option value="">Todos</option>
                    </select>
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-[9px] font-black text-slate-400 uppercase">Categoría</label>
                    <select id="filtro-categoria" class="text-xs border-slate-200 rounded-xl px-3 py-2.5 font-bold text-slate-700 bg-slate-50 min-w-[100px]">
                        <option value="">Todas</option>
                    </select>
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-[9px] font-black text-slate-400 uppercase">Progresión</label>
                    <select id="filtro-etapa" class="text-xs border-slate-200 rounded-xl px-3 py-2.5 font-bold text-slate-700 bg-slate-50">
                        <option value="">Todas</option>
    ...
    <div id="seccion-gantt" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
            ...
            <span class="text-[9px] text-slate-400 ml-auto font-semibold uppercase tracking-tighter">Borrador de Trabajo</span>
        </div>
    ...
    <div id="seccion-tabla" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-[11px] font-black text-slate-600 uppercase tracking-widest">Lista de Establecimientos — Edición</h3>
            </div>
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-left px-3 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">EESS</th>
...
<div id="modal-edit" class="fixed inset-0 z-[9999] items-center justify-center bg-slate-900/60 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-sm mx-4 overflow-hidden">
        ...
        <form id="modal-form" class="px-6 py-5 space-y-4">
            @csrf
            <div>
                <label class="text-[10px] font-black text-slate-500 uppercase tracking-wider block mb-1">Nuevo Sector Asignado</label>
                <select id="modal-sector" name="sector" class="w-full text-sm border border-slate-200 rounded-xl px-3 py-2.5 font-bold text-slate-700 bg-slate-50 focus:ring-indigo-500 focus:border-indigo-400">
                    @for($s = 1; $s <= 23; $s++)
                        <option value="{{ $s }}">Sector {{ $s }}</option>
                    @endfor
                </select>
                <p class="text-[9px] text-slate-400 mt-1 italic">El color del punto en el mapa cambiará automáticamente.</p>
            </div>
            <div>
                <label class="text-[10px] font-black text-slate-500 uppercase tracking-wider block mb-1">Cuadrilla (Borrador)</label>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-1 px-4 py-2.5 text-sm font-black text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200">
                    Guardar Propuesta
                </button>
            </div>
        </form>
