...
@section('content')
  ...
  <div class="max-w-5xl mx-auto py-6 px-3">
    ...
    <form action="{{ route('usuario.monitoreo.citas.create', $acta->id) }}" method="POST" enctype="multipart/form-data" id="mainForm">
      @csrf
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
            ...
            <div class="md:col-span-3">
              ...
              <select name="contenido[personal_turno]" class="input-blue text-center">
                <option value="MAÑANA" {{ ($registro->personal_turno ?? '') == 'MAÑANA' ? 'selected' : '' }}>MAÑANA</option>
                <option value="TARDE" {{ ($registro->personal_turno ?? '') == 'TARDE' ? 'selected' : '' }}>TARDE</option>
                <option value="NOCHE" {{ ($registro->personal_turno ?? '') == 'NOCHE' ? 'selected' : '' }}>NOCHE</option>
              </select>
            </div>
              ...
              <div id="rol-dropdown-list"
                ...
                <div class="space-y-1">
                  @foreach (['ADMISIONISTA', 'CAJERO'] as $rol)
                    <label class="flex items-center gap-2 p-2 hover:bg-slate-50 rounded cursor-pointer border-b border-slate-50 last:border-0">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
              ...
              <div class="md:col-span-4">
                  ...
                  <div class="relative">
                      ...
                      <input type="email" name="contenido[personal_correo]" id="personal_correo"
                          class="input-blue bg-slate-50 text-slate-600 font-regular" 
                          style="padding-left: 2.8rem;" 
                          placeholder="Escribe aqui..."
                          value="{{ $registro->personal_correo ?? '' }}">
                  </div>
              </div>
              ...
              <div class="md:col-span-4">
                  ...
                  <div class="relative">
                      ...
                      <input type="text" name="contenido[personal_celular]" id="personal_celular"
                          ...
                          style="padding-left: 2.8rem;"
                          placeholder="Escribe aqui..."
                          value="{{ $registro->personal_celular ?? '' }}">
                  </div>
              </div>
              ...
              <div class="md:col-span-4">
                  ...
                  <div class="relative">
                      ...
                      <input type="text" name="contenido[personal_cargo]" id="personal_cargo"
                          class="input-blue font-regular border-indigo-200 focus:border-indigo-500 text-indigo-700"
                          style="padding-left: 2.8rem;"
                          placeholder="Escribe aqui..."
                          value="{{ $registro->personal_cargo ?? '' }}">
                  </div>
              </div>
          </div>
      ...
      <div class="form-card section-container mt-6">
              ...
              <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                  <div class="flex items-center gap-3">
                      ...
                      <div>
                          ...
                          <p class="text-[10px] text-slate-500 uppercase font-bold mt-1">Habilita opciones de seguridad informática</p>
                      </div>
                  </div>
                  
                  <div class="toggle-group">
                      <label>
                          <input type="radio" name="contenido[utiliza_sihce]" value="SI" class="toggle-radio"
                              onchange="toggleSihce(true)"
                              {{ ($registro->utiliza_sihce ?? '') == 'SI' ? 'checked' : '' }}>
                          <span class="toggle-btn px-6"><i data-lucide="check" class="w-4 h-4"></i> SÍ</span>
                      </label>
                      <label>
                          <input type="radio" name="contenido[utiliza_sihce]" value="NO" class="toggle-radio"
                              onchange="toggleSihce(false)"
          ...
          <div id="bloque-seguridad-sihce" class="{{ ($registro->utiliza_sihce ?? '') == 'SI' ? '' : 'hidden' }} animate-fadeIn">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group hover:border-indigo-300 transition-colors">
                      ...
                      <div class="toggle-group w-full flex justify-center">
                          <label class="flex-1">
                              ...
                              <span class="toggle-btn w-full justify-center"><i data-lucide="check" class="w-4 h-4 mr-2"></i> SÍ</span>
                          </label>
                          <label class="flex-1">
                              <input type="radio" name="contenido[firma_dj]" value="NO" class="toggle-radio"
                                  {{ ($registro->firma_dj ?? '') == 'NO' ? 'checked' : '' }}>
                              <span class="toggle-btn w-full justify-center"><i data-lucide="x" class="w-4 h-4 mr-2"></i> NO</span>
                          </label>
                      </div>
                  </div>
                  ...
                  <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group hover:border-indigo-300 transition-colors">
                      <div class="absolute top-0 left-0 w-1 h-full bg-indigo-500 rounded-l-xl"></div>
                      <p class="text-sm font-bold text-slate-700 mb-3 pl-3">¿Firmó compromiso de confidencialidad?</p>
                      <div class="toggle-group w-full flex justify-center">
                          <label class="flex-1">
             ...
             <label class="cursor-pointer relative group">
                ...
                <div class="p-5 rounded-xl border-2 transition-all duration-200 border-slate-200 bg-white hover:border-indigo-300 peer-checked:border-indigo-600 peer-checked:bg-indigo-50/30">
                  <div class="flex items-center gap-4">
                    ...
                    <div><h4 class="font-bold text-slate-800">DNI ELECTRÓNICO</h4><span class="text-xs font-medium text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded">CON CHIP</span></div>
                ...
                <div>
                  ...
                  <div class="relative bg-white rounded-lg">
                    <select name="contenido[dnie_version]" class="input-blue w-full">
                      ...
                      <option value="1.0" {{ ($registro->dnie_version ?? '') == '1.0' ? 'selected' : '' }}>VERSIÓN 1.0</option>
                      <option value="2.0" {{ ($registro->dnie_version ?? '') == '2.0' ? 'selected' : '' }}>VERSIÓN 2.0</option>
                      <option value="3.0" {{ ($registro->dnie_version ?? '') == '3.0' ? 'selected' : '' }}>VERSIÓN 3.0</option>
                    </select>
                  </div>
                </div>
                <div>
                  ...
                  <div class="flex items-center gap-4 h-[42px]">
                    <label class="flex items-center cursor-pointer">
                      <input type="radio" name="contenido[firma_sihce]" value="SI" class="w-5 h-5 text-indigo-600 border-gray-300 focus:ring-indigo-500" {{ ($registro->firma_sihce ?? '') == 'SI' ? 'checked' : '' }}>
                      <span class="ml-2 text-sm font-bold text-slate-700">SÍ</span>
                    </label>
                    <label class="flex items-center cursor-pointer">
                      <input type="radio" name="contenido[firma_sihce]" value="NO" class="w-5 h-5 text-indigo-600 border-gray-300 focus:ring-indigo-500" {{ ($registro->firma_sihce ?? '') == 'NO' ? 'checked' : '' }}>
          ...
          <div class="bg-slate-50 p-6 rounded-xl border border-slate-100 mb-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              @foreach (['TICKET', 'FUA', 'HOJA DE FILIACIÓN', 'PAPEL BOND', 'TONER / TINTA', 'LAPICEROS'] as $insumo)
                <label class="flex items-center gap-3 cursor-pointer bg-white p-3 rounded-lg border border-slate-200 hover:border-indigo-400 group transition-colors">
                  ...
                  <span class="text-[11px] font-bold text-slate-600 group-hover:text-indigo-700 uppercase">{{ $insumo }}</span>
                </label>
              @endforeach
            </div>
          </div>
                  ...
                  <tr class="group hover:bg-slate-50 transition-colors">
                    ...
                    <td class="p-2 align-middle">
                      <select name="contenido[equipos][{{ $idx }}][propiedad]"
                        ...
                        <option value="EXCLUSIVO" {{ ($item['propiedad'] ?? '') == 'EXCLUSIVO' ? 'selected' : '' }}>Exclusivo</option>
                        <option value="COMPARTIDO" {{ ($item['propiedad'] ?? '') == 'COMPARTIDO' ? 'selected' : '' }}>Compartido</option>
                        <option value="PERSONAL" {{ ($item['propiedad'] ?? '') == 'PERSONAL' ? 'selected' : '' }}>Personal</option>
                      </select>
                    </td>
                    <td class="p-2 align-middle">
                ...
                <table class="blue-table mb-0" id="tabla-produccion">
                  ...
                  <tbody>
                    @php
                      ...
                      $prodItems = count($produccion) > 0 ? $produccion : [
                    ...
                    @foreach ($prodItems as $i => $item)
                      <tr>
                        ...
                        <td class="text-center"><button type="button" onclick="this.closest('tr').remove()"
                            class="p-1 rounded text-slate-300 hover:text-red-500"><i data-lucide="trash-2" class="w-4 h-4"></i></button></td>
                      </tr>
                    @endforeach
                  </tbody>
                </table>
            ...
            <div class="space-y-8">
              <div>
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                    ...
                    <div class="toggle-group">
                      <label><input type="radio" name="contenido[calidad][espera]" value="SI" class="toggle-radio"
                          {{ ($registro->calidad_tiempo_espera ?? '') == 'SI' ? 'checked' : '' }}><span class="toggle-btn"><i data-lucide="check" class="w-3 h-3"></i> SÍ</span></label>
                      <label><input type="radio" name="contenido[calidad][espera]" value="NO" class="toggle-radio"
                          {{ ($registro->calidad_tiempo_espera ?? '') == 'NO' ? 'checked' : '' }}><span class="toggle-btn"><i data-lucide="x" class="w-3 h-3"></i> NO</span></label>
                    </div>
                  </div>
                  {{-- Pregunta 2 --}}
                  <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                    ...
                    <div class="toggle-group">
                      <label><input type="radio" name="contenido[calidad][satisfaccion]" value="SI" class="toggle-radio"
                          {{ ($registro->calidad_paciente_satisfecho ?? '') == 'SI' ? 'checked' : '' }}><span class="toggle-btn"><i data-lucide="check" class="w-3 h-3"></i> SÍ</span></label>
                      <label><input type="radio" name="contenido[calidad][satisfaccion]" value="NO" class="toggle-radio"
                          {{ ($registro->calidad_paciente_satisfecho ?? '') == 'NO' ? 'checked' : '' }}><span class="toggle-btn"><i data-lucide="x" class="w-3 h-3"></i> NO</span></label>
                    </div>
                  </div>
                  {{-- Pregunta 3 --}}
                  <div class="bg-slate-50 rounded-lg border border-slate-100 p-3">
                    <div class="flex items-center justify-between">
                      ...
                      <div class="toggle-group">
                        <label><input type="radio" name="contenido[calidad][reportes]" value="SI" class="toggle-radio" onchange="toggleReportes(true)"
                            {{ ($registro->calidad_usa_reportes ?? '') == 'SI' ? 'checked' : '' }}><span class="toggle-btn"><i data-lucide="check" class="w-3 h-3"></i> SÍ</span></label>
                        <label><input type="radio" name="contenido[calidad][reportes]" value="NO" class="toggle-radio" onchange="toggleReportes(false)"
                            {{ ($registro->calidad_usa_reportes ?? '') == 'NO' ? 'checked' : '' }}><span class="toggle-btn"><i data-lucide="x" class="w-3 h-3"></i> NO</span></label>
                      </div>
                    </div>
                    <div id="div-reportes-detalle" class="{{ ($registro->calidad_usa_reportes ?? '') == 'SI' ? '' : 'hidden' }} mt-3 pt-3 border-t border-slate-200/50">
              ...
              <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-5 space-y-6">
                  <div>
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3">1. ¿A quién comunica?</p>
                    <div class="grid grid-cols-3 gap-3">
                      @foreach (['MINSA', 'DIRESA', 'OTROS', 'UNIDAD EJECUTORA', 'JEFE DE ESTABLECIMIENTO'] as $opcion)
                        <label class="cursor-pointer group relative">
                          <input type="radio" name="contenido[dificultades][comunica]" value="{{ $opcion }}" class="peer sr-only"
                            {{ ($registro->dificultad_comunica_a ?? '') == $opcion ? 'checked' : '' }}>
                          <div class="text-center py-3 px-2 rounded-lg border border-slate-200 bg-white transition-all peer-checked:border-indigo-500 peer-checked:bg-indigo-50/50 peer-checked:shadow-sm group-hover:border-indigo-300">
                            <span class="block text-xs font-bold text-slate-500 peer-checked:text-indigo-700">{{ $opcion }}</span>
                          </div>
                          <div class="absolute -top-2 -right-2 bg-indigo-600 text-white rounded-full p-0.5 opacity-0 peer-checked:opacity-100 transition-opacity shadow-sm">
                  ...
                  <div>
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3">2. ¿Qué medio utiliza?</p>
                    <div class="grid grid-cols-3 gap-3">
                      @foreach (['WhatsApp', 'Teléfono', 'Email'] as $opcion)
                        <label class="cursor-pointer group relative">
                          <input type="radio" name="contenido[dificultades][medio]" value="{{ $opcion }}" class="peer sr-only"
      ...
      <div class="form-card section-container">
          ...
          <div class="mb-6">
            <div class="flex gap-4 mb-4 border-b border-slate-200">
              <button type="button" onclick="switchTab('local')" id="tab-local"
                class="pb-2 text-sm font-bold text-indigo-600 border-b-2 border-indigo-600 transition-all"><i data-lucide="upload-cloud" class="w-4 h-4 inline mr-1"></i> Subir desde PC</button>
              <button type="button" onclick="switchTab('server')" id="tab-server"
                class="pb-2 text-sm font-bold text-slate-400 hover:text-indigo-500 transition-all"><i data-lucide="server" class="w-4 h-4 inline mr-1"></i> Explorar Servidor</button>
            </div>
            <div id="panel-local" class="block">
              <div class="upload-area relative group bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:bg-indigo-50 hover:border-indigo-400 transition-all cursor-pointer">
                ...
                <div class="flex flex-col items-center gap-3">
                  <div class="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center text-indigo-500 group-hover:scale-110 transition-transform">
          ...
          <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4" id="gallery-container">
              <div id="empty-state" class="col-span-full py-6 text-center text-slate-400 text-xs italic bg-slate-50 rounded-lg border border-dashed border-slate-200">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (fotosGuardadas.length > 0) {
            fotosGuardadas.forEach((url, i) => {
                evidenceList.push({
                    type: 'server', file: null, url: url, name: 'Foto Guardada', id: Date.now() + i
                });
            });
            renderGallery();
            syncInputs();
    ...
    function buscarPorNombre() {
      ...
      timeoutNombre = setTimeout(async () => {
        try {
          const response = await fetch(`{{ route('usuario.monitoreo.citas.buscar.profesional') }}?type=name&q=${query}`);
          const data = await response.json();
          lista.innerHTML = '';
          ...
          if (data.length > 0) {
            ...
            data.forEach(prof => {
              ...
              item.innerHTML = `<strong>${nombreCompleto}</strong> <span class="text-slate-400">(${prof.tipo_doc}: ${prof.doc})</span>`;
              item.onclick = () => {
                rellenarDatos(prof);
                lista.classList.add('hidden');
              };
    ...
    window.toggleRolDropdown = function() { document.getElementById('rol-dropdown-list').classList.toggle('hidden'); }
    ...
    document.addEventListener('click', function(e) {
      ...
      if (dropdown && !button && !dropdown.contains(e.target) && !dropdown.classList.contains('hidden')) dropdown.classList.add('hidden');
    });
    ...
    window.switchTab = function(tab) {
      ...
      if (tab === 'local') {
        panelLocal.classList.remove('hidden'); panelServer.classList.add('hidden');
        btnLocal.classList.add(...activeClass); btnLocal.classList.remove(...inactiveClass);
        btnServer.classList.remove(...activeClass);
      } else {
        panelLocal.classList.add('hidden'); panelServer.classList.remove('hidden');
        btnServer.classList.add(...activeClass); btnServer.classList.remove(...inactiveClass);
        btnLocal.classList.remove(...activeClass);
      }
    }
    window.openServerModal = function() { alert("🚧 MANTENIMIENTO 🚧\nExplorador habilitado en producción."); }
    ...
    window.toggleSihce = function(show) {
        ...
        if(show) bloque.classList.remove('hidden'); else bloque.classList.add('hidden');
    }
    ...
    function agregarEquipoDesdeSelect() {
      ...
      if (!tipoEquipo) { alert("Por favor seleccione un equipo de la lista."); return; }
      const tbody = document.getElementById('tbody-equipos');
      const esOtro = tipoEquipo === 'OTRO';
      const valorNombre = esOtro ? '' : tipoEquipo;
      const inputNombre = `<input type="text" name="contenido[equipos][${equipoIndex}][nombre]" value="${valorNombre}" class="w-full bg-transparent border-0 border-b border-transparent focus:border-indigo-500 focus:ring-0 font-bold text-slate-700 text-xs px-2 py-1 placeholder-slate-300" placeholder="Escriba nombre del equipo..." ${esOtro ? 'autofocus' : ''}>`;
      const fila = `<tr class="group hover:bg-slate-50 transition-colors"><td class="p-2 align-middle">${inputNombre}</td><td class="p-2 align-middle"><div class="relative flex items-center"><input type="text" id="serie-input-${equipoIndex}" name="contenido[equipos][${equipoIndex}][serie]" class="w-full bg-slate-50 border border-slate-200 text-slate-600 text-[11px] font-mono uppercase rounded pl-2 pr-8 py-1 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 placeholder-slate-400" placeholder="----"><button type="button" onclick="iniciarEscaneo('serie-input-${equipoIndex}')" class="absolute right-0.5 p-1 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors cursor-pointer z-10"><i data-lucide="scan-barcode" class="w-3 h-3"></i></button></div></td><td class="p-2 align-middle"><select name="contenido[equipos][${equipoIndex}][propiedad]" class="w-full bg-white border border-slate-200 text-[11px] text-slate-600 rounded px-1 py-1 focus:border-indigo-500 focus:ring-0 cursor-pointer"><option value="EXCLUSIVO" selected>Exclusivo</option><option value="COMPARTIDO">Compartido</option><option value="PERSONAL">Personal</option></select></td><td class="p-2 align-middle"><select name="contenido[equipos][${equipoIndex}][estado]" class="w-full bg-white border border-slate-200 text-[11px] rounded px-1 py-1 focus:border-indigo-500 focus:ring-0 cursor-pointer text-slate-600"><option value="Operativo" selected>Operativo</option><option value="Regular">Regular</option><option value="Inoperativo">Inoperativo</option></select></td><td class="p-2 align-middle"><input type="text" name="contenido[equipos][${equipoIndex}][observaciones]" class="w-full bg-transparent text-[11px] text-slate-500 border-0 border-b border-transparent focus:border-indigo-500 focus:ring-0 placeholder-slate-300 italic px-2 py-1" placeholder="Observaciones..."></td><td class="p-2 text-center align-middle"><button type="button" onclick="this.closest('tr').remove()" class="text-slate-300 hover:text-red-500 hover:bg-red-50 p-1 rounded transition-all"><i data-lucide="trash-2" class="w-3 h-3"></i></button></td></tr>`;
    ...
    window.handleFiles = function(files) {
      ...
      Array.from(files).forEach(file => { if (evidenceList.length >= MAX_PHOTOS) evidenceList.shift(); evidenceList.push({ type: 'local', file: file, url: URL.createObjectURL(file), name: file.name, id: generateId() }); });
      renderGallery(); syncInputs();
    }
    window.removeImage = function(id) { evidenceList = evidenceList.filter(item => item.id !== id); renderGallery(); syncInputs(); }
    function renderGallery() {
      const container = document.getElementById('gallery-container');
      const countDisplay = document.getElementById('count-display');
      const emptyState = document.getElementById('empty-state');
      if (!container) return;
      Array.from(container.children).forEach(c => { if (c.id !== 'empty-state') container.removeChild(c); });
      if (evidenceList.length === 0) { if (emptyState) emptyState.style.display = 'block'; if (countDisplay) countDisplay.innerText = `0 / ${MAX_PHOTOS}`; return; }
      if (emptyState) emptyState.style.display = 'none';
      evidenceList.forEach(item => {
        const div = document.createElement('div');
        div.className = 'relative group aspect-square rounded-xl overflow-hidden border border-slate-200 shadow-sm bg-white animate-fade-in';
        div.innerHTML = `<img src="${item.url}" class="w-full h-full object-cover"><button type="button" onclick="removeImage(${item.id})" class="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full shadow hover:bg-red-600 z-10"><i data-lucide="x" class="w-3 h-3"></i></button><div class="absolute top-2 left-2 ${item.type === 'local' ? 'bg-indigo-500' : 'bg-emerald-500'} text-white text-[9px] font-bold px-2 py-0.5 rounded-full shadow-sm"><span>${item.type.toUpperCase()}</span></div>`;
    ...
    function syncInputs() {
      ...
      const input = document.getElementById('final-input-files'); if (input) input.files = dt.files;
    ...
    let html5QrcodeScanner = null; let currentInputId = null;
    function iniciarEscaneo(inputId) {
      currentInputId = inputId;
      const modal = document.getElementById('scanner-modal'); modal.classList.remove('hidden');
      html5QrcodeScanner = new Html5Qrcode("reader");
      html5QrcodeScanner.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } }, onScanSuccess, () => {}).catch(err => { console.error(err); alert("Error cámara"); modal.classList.add('hidden'); });
    }
    function onScanSuccess(decodedText) { if (currentInputId) document.getElementById(currentInputId).value = decodedText; detenerEscaneo(); }
    function detenerEscaneo() { if (html5QrcodeScanner) html5QrcodeScanner.stop().then(() => { html5QrcodeScanner.clear(); document.getElementById('scanner-modal').classList.add('hidden'); currentInputId = null; }).catch(console.error); else document.getElementById('scanner-modal').classList.add('hidden'); }
    ...
    function toggleDniOptions(tipo) {
      ...
      if (tipo === 'ELECTRONICO') container.classList.remove('hidden'); else container.classList.add('hidden');
    }
