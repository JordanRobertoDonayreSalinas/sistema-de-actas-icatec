...
@section('content')
            ...
            <form action="{{ route('usuario.monitoreo.puerperio.store', $acta->id) }}" method="POST"
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                    <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                        <span
                            class="bg-rose-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">1</span>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                    <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                        <span
                            class="bg-rose-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">2</span>
                ...
                <div id="section_dni_detalle" class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100 hidden">
                    <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                        <span
                            class="bg-rose-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">3</span>
                ...
                <div id="section_capacitacion" class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                    <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                        <span
                            class="bg-rose-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">4</span>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                    <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                        <span
                            class="bg-rose-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">5</span>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                    <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                        <span
                            class="bg-rose-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">6</span>
                        <h3 class="text-rose-900 font-black text-lg uppercase tracking-tight">TIPO DE CONECTIVIDAD</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div id="card_wifi" onclick="selectConectividad('WIFI')"
                            class="cursor-pointer border-2 rounded-2xl p-6 flex items-center gap-4 transition-all hover:shadow-md {{ ($contenido['tipo_conectividad'] ?? '') == 'WIFI' ? 'border-rose-600 bg-rose-50' : 'border-slate-200 bg-white' }}">
                            ...
                            <div>
                                ...
                                <span
                                    class="text-[10px] font-bold text-rose-500 bg-rose-100 px-2 py-0.5 rounded uppercase">Inalámbrico</span>
                            </div>
                        </div>

                        {{-- TARJETA CABLEADO --}}
                        <div id="card_cableado" onclick="selectConectividad('CABLEADO')"
                            class="cursor-pointer border-2 rounded-2xl p-6 flex items-center gap-4 transition-all hover:shadow-md {{ ($contenido['tipo_conectividad'] ?? '') == 'CABLEADO' ? 'border-rose-600 bg-rose-50' : 'border-slate-200 bg-white' }}">
                    ...
                    <div id="bloque_wifi_fuente"
                        ...
                        <label class="block text-rose-600 text-[10px] font-black uppercase tracking-widest mb-4">¿De dónde
                            proviene el WiFi?</label>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {{-- ESTABLECIMIENTO --}}
                            <div id="card_wifi_establecimiento" onclick="selectWifiFuente('ESTABLECIMIENTO')"
                                class="cursor-pointer border-2 rounded-xl p-4 flex items-center gap-3 transition-all hover:shadow-md {{ ($contenido['wifi_fuente'] ?? '') == 'ESTABLECIMIENTO' ? 'border-rose-600 bg-rose-50' : 'border-slate-200 bg-white' }}">
                                <div
                                    class="h-10 w-10 rounded-lg bg-rose-100 flex items-center justify-center text-rose-600">
                                    <i data-lucide="building-2" class="w-5 h-5"></i>
                                </div>
                                <div>
                                    <h4 class="text-xs font-black text-slate-800 uppercase">Establecimiento</h4>
                                    <span class="text-[9px] font-bold text-rose-400">Red del EESS</span>
                                </div>
                            </div>
                            {{-- PERSONAL --}}
                            <div id="card_wifi_personal" onclick="selectWifiFuente('PERSONAL')"
                                class="cursor-pointer border-2 rounded-xl p-4 flex items-center gap-3 transition-all hover:shadow-md {{ ($contenido['wifi_fuente'] ?? '') == 'PERSONAL' ? 'border-rose-600 bg-rose-50' : 'border-slate-200 bg-white' }}">
                                <div
                ...
                <div id="section_soporte"
                    class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100 {{ ($contenido['cuenta_sihce'] ?? '') == 'NO' ? 'hidden' : '' }}">
                    <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                        <span
                            class="bg-rose-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">7</span>
    ...
    <script>
        ...
        function selectConectividad(tipo) {
            ...
            if (tipo === 'WIFI') {
                cardWifi.classList.add('border-rose-600', 'bg-rose-50');
                cardWifi.classList.remove('border-slate-200', 'bg-white');
                cardCableado.classList.remove('border-rose-600', 'bg-rose-50');
                cardCableado.classList.add('border-slate-200', 'bg-white');
                bloqueWifiFuente.classList.remove('hidden');
            } else {
                cardCableado.classList.add('border-rose-600', 'bg-rose-50');
                cardCableado.classList.remove('border-slate-200', 'bg-white');
                cardWifi.classList.remove('border-rose-600', 'bg-rose-50');
                ...
                document.getElementById('card_wifi_establecimiento').classList.remove('border-rose-600', 'bg-rose-50');
                document.getElementById('card_wifi_establecimiento').classList.add('border-slate-200', 'bg-white');
                document.getElementById('card_wifi_personal').classList.remove('border-rose-600', 'bg-rose-50');
        ...
        function selectWifiFuente(fuente) {
            ...
            if (fuente === 'ESTABLECIMIENTO') {
                cardEstablecimiento.classList.add('border-rose-600', 'bg-rose-50');
                cardEstablecimiento.classList.remove('border-slate-200', 'bg-white');
                cardPersonal.classList.remove('border-rose-600', 'bg-rose-50');
                cardPersonal.classList.add('border-slate-200', 'bg-white');
            } else {
                cardPersonal.classList.add('border-rose-600', 'bg-rose-50');
                cardPersonal.classList.remove('border-slate-200', 'bg-white');
                cardEstablecimiento.classList.remove('border-rose-600', 'bg-rose-50');
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            console.log('Initializing connectivity:', conectividadVal); // DEBUG LOG
