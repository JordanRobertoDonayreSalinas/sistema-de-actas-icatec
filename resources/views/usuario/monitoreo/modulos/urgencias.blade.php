...
<div class="py-12 bg-slate-50 min-h-screen">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 font-bold uppercase text-xs mt-1">
                    <i data-lucide="siren" class="inline-block w-4 h-4 mr-1 text-red-500"></i> {{ $acta->establecimiento->nombre }}
                </p>
            </div>
        ...
        <form action="{{ route('usuario.monitoreo.urgencias.store', $acta->id) }}" 
              ...
              class="space-y-6" 
              id="form-monitoreo-urgencias">
            @csrf
            
            {{-- 1.- DATOS GENERALES --}}
            <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                    <span class="bg-red-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">1</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    ...
                    <div>
                        ...
                        <input type="date" name="contenido[fecha]" value="{{ $detalle->contenido['fecha'] ?? date('Y-m-d') }}" class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-slate-800 font-bold outline-none focus:border-red-500 transition-all">
                    </div>
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                    <span class="bg-red-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">2</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-slate-100">
                    {{-- 1. Utiliza SIHCE --}}
                    <div>
                        <label class="block text-red-600 text-[10px] font-black uppercase tracking-widest mb-2">¿Utiliza SIHCE?</label>
                        <select name="contenido[cuenta_sihce]" id="cuenta_sihce" onchange="toggleSihceAndDocs(this.value)" class="w-full px-4 py-3 bg-red-50 border-2 border-red-100 rounded-xl font-bold text-sm uppercase outline-none text-red-700 cursor-pointer hover:bg-red-100 transition-colors">
                    ...
                    <div id="div_firmo_dj">
                        ...
                        <select name="contenido[firmo_dj]" id="firmo_dj" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-sm uppercase outline-none focus:border-red-500">
                    ...
                    <div id="div_firmo_confidencialidad">
                        ...
                        <select name="contenido[firmo_confidencialidad]" id="firmo_confidencialidad" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-sm uppercase outline-none focus:border-red-500">
            ...
            <div id="seccion_detalle_dni" class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100 transition-all duration-300">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                    <span class="bg-red-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">3</span>
                    <h3 class="text-red-900 font-black text-lg uppercase tracking-tight">DETALLE DE DNI Y FIRMA DIGITAL</h3>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div id="card_electronico" onclick="selectDniType('ELECTRONICO')" 
                             class="cursor-pointer border-2 rounded-2xl p-6 flex items-center gap-4 transition-all hover:shadow-md {{ ($detalle->contenido['tipo_dni'] ?? '') == 'ELECTRONICO' ? 'border-red-600 bg-red-50' : 'border-slate-200 bg-white' }}">
                            ...
                            <div>
                                ...
                                <span class="text-[10px] font-bold text-red-500 bg-red-100 px-2 py-0.5 rounded uppercase">Con Chip</span>
                            </div>
                        </div>

                        {{-- DNI AZUL --}}
                        <div id="card_azul" onclick="selectDniType('AZUL')" 
                             class="cursor-pointer border-2 rounded-2xl p-6 flex items-center gap-4 transition-all hover:shadow-md {{ ($detalle->contenido['tipo_dni'] ?? '') == 'AZUL' ? 'border-red-600 bg-red-50' : 'border-slate-200 bg-white' }}">
                ...
                <div id="bloque_opciones_dni" class="bg-slate-50 rounded-2xl p-6 border border-slate-200 {{ empty($detalle->contenido['tipo_dni']) ? 'hidden' : '' }}">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div id="bloque_version_dnie" class="{{ ($detalle->contenido['tipo_dni'] ?? '') == 'ELECTRONICO' ? '' : 'hidden' }}">
                            <label class="block text-red-600 text-[10px] font-black uppercase tracking-widest mb-2">Versión del DNIe</label>
                            <select name="contenido[version_dnie]" class="w-full px-4 py-3 bg-white border-2 border-red-100 rounded-xl font-bold text-sm uppercase outline-none focus:border-red-500 text-red-700">
                        ...
                        <div id="bloque_firma_digital" class="{{ ($detalle->contenido['tipo_dni'] ?? '') == 'ELECTRONICO' ? '' : 'hidden' }}">
                            <label class="block text-red-600 text-[10px] font-black uppercase tracking-widest mb-3">¿Firma Digitalmente en SIHCE?</label>
                            <div class="flex items-center gap-6">
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <div class="relative flex items-center">
                                        <input type="radio" name="contenido[firma_digital_sihce]" value="SI" class="peer h-5 w-5 cursor-pointer appearance-none rounded-full border-2 border-slate-300 checked:border-red-600 transition-all" {{ ($detalle->contenido['firma_digital_sihce'] ?? '') == 'SI' ? 'checked' : '' }}>
                                        <span class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-red-600 rounded-full opacity-0 peer-checked:opacity-100 transition-opacity"></span>
                                    </div>
                                    <span class="text-sm font-bold text-slate-600 group-hover:text-red-600 transition-colors">SÍ</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <div class="relative flex items-center">
                        ...
                        <div class="md:col-span-2 mt-2">
                            ...
                            <textarea name="contenido[observaciones_dni]" rows="2" class="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-sm uppercase outline-none focus:border-red-500 placeholder-slate-300" placeholder="Escriba aquí si presenta dificultades...">{{ $detalle->contenido['observaciones_dni'] ?? '' }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                    <span class="bg-red-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">4</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="contenido[recibio_capacitacion]" id="recibio_capacitacion" onchange="toggleEntidadCapacitadora(this.value)" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-red-500 transition-all uppercase cursor-pointer">
                    ...
                    <div id="wrapper_entidad_capacitadora" class="hidden md:col-span-2">
                        ...
                        <select name="contenido[inst_que_lo_capacito]" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-red-500 transition-all uppercase cursor-pointer">
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                    <span class="bg-red-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">5</span>
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                    <span class="bg-red-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">6</span>
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                    <span class="bg-red-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">7</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-slate-500 text-[10px] font-black uppercase tracking-widest mb-2">Prioridad de Triaje Predominante</label>
                        <select name="contenido[triaje_prioridad]" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-red-500 transition-all uppercase cursor-pointer">
                            <option value="PRIORIDAD III" {{ ($detalle->contenido['triaje_prioridad'] ?? '') == 'PRIORIDAD III' ? 'selected' : '' }}>PRIORIDAD III</option>
                            <option value="PRIORIDAD IV" {{ ($detalle->contenido['triaje_prioridad'] ?? '') == 'PRIORIDAD IV' ? 'selected' : '' }}>PRIORIDAD IV</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-slate-500 text-[10px] font-black uppercase tracking-widest mb-2">Tiempo Promedio de Espera</label>
                        <select name="contenido[tiempo_espera]" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-red-500 transition-all uppercase cursor-pointer">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div>
                        <div class="flex items-center gap-3 mb-6">
                            <span class="bg-red-500 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">8</span>
                            <h3 class="text-white font-black text-lg uppercase tracking-tight">COMENTARIOS</h3>
                        </div>
                        <textarea name="contenido[comentarios]" rows="6" class="w-full bg-white/10 border-2 border-white/20 rounded-2xl p-4 text-white font-bold outline-none focus:border-red-500 transition-all uppercase placeholder-white/30">{{ $detalle->contenido['comentarios'] ?? '' }}</textarea>
                    </div>
                    <div>
                        <div class="flex items-center gap-3 mb-6">
                            <span class="bg-red-500 text-white w-8 h-8 flex items-center justify-center rounded-full font-black text-sm">9</span>
                        ...
                        <div class="relative group">
                            ...
                            <div id="dropzone" class="bg-white/5 border-2 border-dashed border-white/20 rounded-[2rem] p-8 flex flex-col items-center justify-center group-hover:bg-white/10 transition-all shadow-inner h-48 border-spacing-4">
                                ...
                                <img id="img-preview" src="#" class="hidden mt-2 w-20 h-20 object-cover rounded-lg border-2 border-red-500">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <button type="submit" id="btn-submit-action" 
                    ...
                    <div class="flex items-center gap-8 pointer-events-none">
                        ...
                        <div class="text-left">
                            ...
                            <p class="text-[10px] text-red-200 font-bold uppercase mt-3 tracking-widest">Sincronizar Módulo Urgencias</p>
                        </div>
                    </div>
