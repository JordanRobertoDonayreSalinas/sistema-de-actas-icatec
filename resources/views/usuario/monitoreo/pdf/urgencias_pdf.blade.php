...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Urgencias - Acta {{ $acta->id }}</title>
    <style>
        @page { margin: 1.2cm 1.5cm; }
        body { font-family: 'Helvetica', sans-serif; font-size: 10px; color: #1e293b; line-height: 1.4; }
        .header { text-align: center; margin-bottom: 15px; border-bottom: 2px solid #e11d48; padding-bottom: 10px; }
        .header h1 { margin: 0; font-size: 16px; text-transform: uppercase; color: #e11d48; }
        .section-title { background-color: #f1f5f9; padding: 6px 10px; font-weight: bold; text-transform: uppercase; border-left: 4px solid #e11d48; margin-top: 15px; margin-bottom: 5px; font-size: 10px; }
        ...
        .firma-box { width: 50%; border: 1px solid #94a3b8; border-radius: 10px; display: inline-block; padding: 15px 10px; text-align: center; background-color: #ffffff; }
...
<body>
    ...
    <div class="section-title">1. Responsable del Área de Urgencias</div>
    <table>
        <tr>
    ...
    <div class="section-title">2. Evaluación de Procesos (Datos del Sistema)</div>
    <table>
        <tr>
            <td class="bg-label">Prioridad de Triaje:</td>
        ...
        <tr>
            <td class="bg-label">Disponibilidad de Insumos Críticos:</td>
            <td class="uppercase">{{ $detalle->contenido['insumos_emergencia'] ?? '---' }}</td>
        </tr>
    </table>

    <div class="section-title">3. Inventario de Equipamiento Tecnológico</div>
    <table>
        <thead>
            <tr>
    ...
    <div style="border: 1px solid #e2e8f0; padding: 10px; min-height: 40px; font-size: 9px;" class="uppercase">
        {{ $detalle->contenido['observaciones'] ?? 'SIN OBSERVACIONES ADICIONALES.' }}
    </div>

    <div class="section-title">6. Firma de Conformidad</div>
    <div class="firmas-wrapper">
        <div class="firma-box">
            ...
            <div class="firma-info">RESPONSABLE DE URGENCIAS</div>
